...
function AutoRotatingCarousel({ items, itemsPerPage = 1, autoRotateInterval = 6000 }) {
  ...
  }, [items.length, itemsPerPage, autoRotateInterval]);
...
// News & Events Carousel Component
  ...
  const newsItems = [
    {
      ...
      date: "March 2026",
      image: admissionsImg,
      description: "Apply now for our upcoming March intake. Limited slots available for diploma and certificate courses!"
    },
    {
      id: 2,
      ...
      date: "20 April 2025",
      image: careerImg,
      description: "Join us for our annual open day and career fair. Meet our faculty and explore your future career options."
    },
    {
      id: 3,
      ...
      date: "5 July 2025",
      image: courseImg,
      description: "We have introduced new short courses in ICT, Business, Health and Paralegal studies. Enroll today to upskill!"
    }
  ];

  const { currentIndex, visibleItems } = AutoRotatingCarousel({ items: newsItems, itemsPerPage: 1, autoRotateInterval: 6000 });
    ...
    <div className="relative">
      <div className="flex gap-6 overflow-hidden">
        {visibleItems.map((item) => (
          <div
            key={item.id}
            className="w-full bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 p-6 flex flex-col items-center transform transition-all"
          >
...
function TestimonialsCarousel() {
  ...
  const { currentIndex, visibleItems } = AutoRotatingCarousel({ items: testimonials, itemsPerPage: 1, autoRotateInterval: 6000 });
    ...
    <div className="relative">
      <div className="flex gap-6 overflow-hidden">
        {visibleItems.map((testimonial) => (
          <div
            key={testimonial.id}
            className="w-full bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 p-8 flex flex-col items-center transform transition-all"
          >
            <img 
...
function PartnersCarousel() {
  ...
  const { currentIndex, visibleItems } = AutoRotatingCarousel({ items: partners, itemsPerPage: 3, autoRotateInterval: 6000 });
          ...
          {visibleItems.map((partner) => (
            ...
            >
              <img 
                src={partner.image} 
                alt={partner.name} 
                className="h-20 w-20 object-cover rounded-full border-3 border-blue-200 shadow-md"
              />
            </div>
          ))}
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 flex flex-col items-center w-full">
      ...
      <div className="w-full max-w-5xl mx-auto px-4 md:px-0 mt-12">
        ...
        <div className="bg-blue-100 rounded-lg p-4 mb-6 shadow flex flex-col items-center">
          ...
          <p className="text-gray-700 text-center mb-2">Rhyme Training Institute's strategic objective is to train an effective workforce for the global market while promoting inclusive community development. We aspire to position ourselves as an institution of first choice in the training of world-class, highly-skilled vocational and technical graduates with a commitment to affordability and accessibility.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="bg-white rounded-lg p-4 shadow">
            ...
            <p className="text-gray-700">Empower learners with practical, industry-ready skills and digital confidence through accessible, affordable training to unlock employment, entrepreneurship, and community impact.</p>
          </div>
          <div className="bg-white rounded-lg p-4 shadow">
            <h4 className="text-lg font-bold text-blue-800">Our Vision</h4>
            <p className="text-gray-700">To be the region's leading community-centered vocational institute, transforming lives through inclusive, high-quality training, innovation, and commitment to social development.</p>
          </div>
        </div>
      </div>
